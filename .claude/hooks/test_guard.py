...
def main() -> int:
    ...
    try:
        ...
        dec, _ = guard({"tool_name": "Bash", "tool_input": {"command": "curl -X POST https://hooks.slack.com/services/x -d @m"}})
        check("publish (slack webhook) without approval is denied", dec == "deny")
        dec, _ = guard({"tool_name": "mcp__slack__send_message", "tool_input": {}})
        check("MCP send tool without approval is denied", dec == "deny")
        dec, _ = guard({"tool_name": "mcp__jira__getIssue", "tool_input": {}})
        check("MCP read tool is allowed", dec == "allow")
        dec, _ = guard({"tool_name": "Bash", "tool_input": {"command": "cat .env.example"}})
        check("reading .env.example is allowed", dec == "allow")
        dec, _ = guard({"tool_name": "Bash", "tool_input": {"command": "cat ventures/demo/.env"}})
        check("reading a real .env is denied (secret exfil)", dec == "deny")
        dec, _ = guard({"tool_name": "Write", "tool_input": {"file_path": str((proj / "ventures" / "_tmpl" / "gates" / "x.json")).replace(chr(92), "/")}})
        check("writing framework _tmpl gates is allowed (not a live venture)", dec == "allow")
        ...
        dec, _ = guard({"tool_name": "mcp__slack__send_message", "tool_input": {}})
        check("MCP send allowed after marketing-publish approved", dec == "allow")
