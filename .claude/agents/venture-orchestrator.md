...
1. **Create / resume.** New venture: copy `ventures/_template/` to `ventures/<slug>/` and fill `manifest.json` (slug, title, one_liner, budget, dates) plus a concise **`search_theme`** — a keyword phrase (e.g. `"options trading tax australia"`), NOT the prose one_liner, so the G0 harvest queries cleanly. Resume: read `manifest.json` + run `python .claude/hooks/gate.py status <slug>` to see where it stands. Always set `VF_ACTIVE_VENTURE=<slug>` before delegating so the guard targets the right venture.
2. **G0 Scout.** First **harvest the signal yourself and pass it to the scout** (the reliable channel — you have Bash and the resolved slug; a subagent's skill-injection env may not carry the active venture). With `VF_ACTIVE_VENTURE=<slug>` set, run `python .claude/skills/signal-harvest/scripts/harvest_signals.py` and `python .claude/skills/signal-harvest/scripts/hot_threads.py --json`; capture the harvest digest + the **buyer verdict** (SEARCHABLE / SPARSE / SPARSE-DEGRADED) + top hot threads. Then delegate to `opportunity-scout`, **including that signal + verdict in the delegation prompt** so it routes the demand gate (SEARCHABLE → desk-sizing; SPARSE → outreach kill test) without re-harvesting. Then `python .claude/hooks/gate.py check <slug> G0`.
