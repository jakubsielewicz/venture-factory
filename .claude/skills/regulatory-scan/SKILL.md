...
2. For each, identify: required licences/registrations; sector rules (health / finance / legal / children → high scrutiny); consumer-protection and tax/GST obligations; and data-privacy duties (e.g. GDPR, Australian Privacy Act). Cite the specific rule or authority page with a date. For AU claims: attempt the primary fetch (.gov.au, AustLII); on timeout/403, escalate to **≥2 named law-firm secondaries** citing the same fact, label it "[secondary — Firm A, Firm B]" with a "primary fetch failed; verify before launch" note — never mark a claim UNVERIFIABLE solely because the primary URL was unreachable.
